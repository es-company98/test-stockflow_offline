...
async function loadAppConfig() {
  try {
    ...
    document.getElementById("shopNameValue").textContent =
      data.shopName || "-";

    document.getElementById("shopAddressValue").textContent =
      data.shopAddress || "-";

    document.getElementById("shopPhoneValue").textContent =
      data.shopPhone || "-";

    document.getElementById("currencyValue").textContent =
      data.currency || "-";

    document.getElementById("currencySymbolValue").textContent =
      data.currencySymbol || "-";

    document.getElementById("lowStockValue").textContent =
      data.lowStockLimit || 0;

    document.getElementById("offlineValue").textContent =
      data.enableOffline ? "Activé" : "Désactivé";
...
async function updateAppConfig() {
  try {
    ...
    const confirmUpdate = confirm(
      "⚠️ Voulez-vous vraiment mettre à jour la configuration système ?"
    );

    if (!confirmUpdate) return;

    // IMPORTANT : ici on réécrit uniquement les champs modifiables
